...
class SGLangRollout(BaseRollout):
    ...
    def _batch_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        # Extract token IDs and image/video data for SGLang Engine
        idx_list = [input_data["prompt_token_ids"] for input_data in sglang_inputs]
        image_list = [input_data.get("image_data", None) for input_data in sglang_inputs]
        video_list = [input_data.get("video_data", None) for input_data in sglang_inputs]
    ...
    def _preprocess_prompt_to_async_rollout_requests(self, prompts: DataProto, n: int = 1) -> list[AsyncRolloutRequest]:
        ...
        for data_idx, (raw_prompt, multi_modal_data) in enumerate(
            ...
            if multi_modal_data is not None:
                ...
                def _recursive_tuple_to_list(obj):
                    if isinstance(obj, tuple):
                        return [_recursive_tuple_to_list(x) for x in obj]
                    if isinstance(obj, list):
                        return [_recursive_tuple_to_list(x) for x in obj]
                    return obj

                for key, value in multi_modal_data.items():
                    multi_modal_data[key] = _recursive_tuple_to_list(value)

            # import pdb; pdb.set_trace()

            # Sanitize video data: unwrap [video_tensor, meta, ...] structure if present
            # if multi_modal_data is not None and "video" in multi_modal_data:
            #     if isinstance(multi_modal_data["video"], tuple):
            #         multi_modal_data["video"], multi_modal_data["video_metadata"], multi_modal_data["video_kwarg"] = multi_modal_data
