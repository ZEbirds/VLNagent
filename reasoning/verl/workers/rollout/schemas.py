...
class AsyncRolloutRequest(BaseModel):
    ...
    def _handle_apply_chat_template(
        ...
        if isinstance(processing_class, PreTrainedTokenizer) or isinstance(processing_class, PreTrainedTokenizerFast):
            ...
            if videos:
                videos, video_metadatas, video_kwargs = zip(*videos)
                video_kwargs = video_kwargs[0]
                if isinstance(videos, tuple):
                    videos = videos[0]
                if isinstance(video_metadatas, tuple):
                    video_metadatas = video_metadatas[0]
                if isinstance(video_kwargs, tuple):
                    video_kwargs = video_kwargs[0]
            else:
                video_kwargs, video_metadatas = {}, {}
            model_inputs = processing_class(text=[raw_prompt], images=images, videos=videos, **video_kwargs, video_metadata=video_metadatas, do_resize=True, return_tensors="pt")
        else:
